...
<div class="table-responsive container mt-5 rounded w-75" style="background-color:   #cfcfcfb8">
    ...
    <select class="form-select my-5" name="user_id" id="user-select">
    ...
    <?php foreach ($users as $user): ?>
        ...
        <?php foreach ($orders as $order):
            if ($order["user_id"] == $user['id']) {
                ...
                echo "<tr class = 'orderTable' data-user-id='{$user["id"]}'  style='display: none'>";
                echo '<td colspan="2">';
...
<script>
    ...
    userSelect.addEventListener('change', () => {
        ...
        if (selectedValue > 0) {
            ...
            rows.forEach(row => {
                if (row.dataset.userId === selectedValue) {
                ...
                orderItemsTables.forEach(tableItem => {
                    ...
                    if (DateFrom.value !== "" && DateTo.value !== "") {
                        if (tableItem.childNodes[0].innerText.split(" ")[0] >= DateFrom.value && tableItem.childNodes[0].innerText.split(" ")[0] <= DateTo.value) {
                            ...
                            tableItem.nextSibling.style.display = 'table-row';
                            tableItem.parentElement.parentElement.style.display = 'table';
                            // orderItemsTables.forEach(table => table.style.display = 'table-row');
                        } else {
                            tableItem.style.display = 'none';
                            tableItem.parentElement.parentElement.style.display = 'none';
                            // orderItemsTables.forEach(table => table.style.display = 'none');
                        }
